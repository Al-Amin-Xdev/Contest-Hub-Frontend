...
const AddContest = () => {
  ...
  const onSubmit = async (data) => {
    ...
    // Build the contest object
    const contestData = {
      ...data,
      creatorEmail: user?.email,
      deadline: deadline,       // store as Date
      status: "pending",        // default: admin approval needed
      winStatus: null,          // no winner yet
      createdAt: new Date(),    // timestamp
    };

    try {
      // Send contest data to backend
      const response = await axiosSecure.post("/all-contests", contestData);
