...
// Dummy data simulating backend contests
const dummyContests = [
  {
    id: "1",
    name: "Logo Design Challenge",
    image: "https://via.placeholder.com/400x200.png?text=Logo+Contest",
    description: "Create a unique logo for our brand.",
    price: 10,
    prize: 100,
    taskInstruction: "Design logo using vector software.",
    contestType: "design",
    deadline: new Date(),
  ...
  },
  {
    ...
    image: "https://via.placeholder.com/400x200.png?text=Business+Contest",
    description: "Submit a detailed business plan.",
    price: 15,
    prize: 200,
    taskInstruction: "Submit PDF document.",
...
const EditContest = () => {
  ...
  const onSubmit = (data) => {
    ...
    const updatedContest = { ...editingContest, ...data, deadline };
    setContests((prev) =>
      prev.map((c) => (c.id === editingContest.id ? updatedContest : c))
    );
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-4 md:p-8">
      <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-6">My Contests</h1>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {contests.map((contest) => (
          <div key={contest.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg overflow-hidden flex flex-col">
            <img
              src={contest.image}
              alt={contest.name}
            ...
            <div className="p-4 flex-1 flex flex-col">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-1">{contest.name}</h2>
              <p className="text-gray-700 dark:text-gray-300 text-sm mb-1">{contest.description}</p>
              ...
              <div className="flex items-center gap-2 mb-3">
                {contest.status === "pending" && <FaTimesCircle className="text-yellow-500" />}
                {contest.status === "confirmed" && <FaCheckCircle className="text-green-500" />}
                {contest.status === "rejected" && <FaTimesCircle className="text-red-500" />}
      ...
      {editingContest && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-3xl w-full p-6 md:p-10 overflow-y-auto max-h-[90vh]">
            <h2 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Edit Contest</h2>
            <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 md:grid-cols-2">

              {/* Name */}
              <div className="col-span-2">
                <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">Contest Name</label>
                <input
                  type="text"
              ...
              {/* Price */}
              <div>
                <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">Price ($)</label>
                <input
                  type="number"
              ...
              {/* Prize */}
              <div>
                <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">Prize ($)</label>
                <input
                  type="number"
              ...
              {/* Task Instruction */}
              <div className="col-span-2">
                <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">Task Instruction</label>
                <textarea
              ...
              {/* Contest Type */}
              <div>
                <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">Contest Type</label>
                <select
              ...
              {/* Deadline */}
              <div>
                <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">Deadline</label>
                <DatePicker
                  selected={deadline}
